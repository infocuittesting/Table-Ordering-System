...
def Get_Order_Item_Table(request):
   ...
   get_orders=json.loads(dbget("select food_offers.*,offer_type.offer_type,offer_status.offer_status, \
   order_status.order_status_desc,food_menu.food_name,food_menu.price, \
   ...
   total_amount = json.loads(dbget("select sum(food_menu.price*quantity) as total from food_order \
                                   left join food_menu on food_menu.food_id = food_order.food_id \
                                   and food_order.order_status_id != 7 \
                                   and food_menu.item_category_id!=7 \
                                   where food_order.table_no = 1 "))
